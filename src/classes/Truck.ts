...
class Truck extends Vehicle implements AbleToTow {
  ...
  tow(vehicle: Truck | Motorbike | Car): void {
    ...
    if (vehicle.weight <= this.towingCapacity) {
      console.log(
        `Truck ${this.make} ${this.model} is towing ${vehicleDescription}.`
      );
    } else {
      console.log(
        `Truck ${this.make} ${this.model} cannot tow ${vehicleDescription} because it exceeds the towing capacity.`
      );
    }
  }
